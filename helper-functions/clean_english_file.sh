...
# Copyright 2024
# This script cleans English word files by removing empty lines.

# Description:
# This function removes empty lines from English word files.
# It uses a temporary file to perform the operation safely.

# Usage:
#   clean_english_file "word"
...
clean_english_file() {
  ...
  local source_file="${ENGLISH_DIRECTORY_PATH}/${word}.txt"
  local error_count=0
  ...
  if [[ -z "${ENGLISH_DIRECTORY_PATH}" ]]; then
    echo "Error: ENGLISH_DIRECTORY_PATH environment variable must be defined" >&2
    return 1
  fi
